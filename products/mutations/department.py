from django.template.defaultfilters import slugify
import graphene

from core.utils.decorators import permission_required, role_required
from core.utils.mutations import ModelMutation, ModelStatusChangeMutation
from core.types import Upload
from core.utils.utils import validate_image_file, clean_seo_fields
from core.enums.enum import UserType
from search.schema import SeoInput
from ..utils.thumbnails import create_department_default_thumbnails
from .. import models


class DepartmentInput(graphene.InputObjectType):
    name = graphene.String(
        required=True, description="Department name")
    note = graphene.String(description='Department note.')
    url = graphene.String(description='Department url.')
    icon = Upload(description='Icon image file.')
    large_icon = Upload(description='Large icon image file.')
    default_image = Upload(description='Default image file.')
    image_alt_text = graphene.String(description='Image alt text .')
    seo = SeoInput(description='Search engine optimization fields.')
    view_type = graphene.ID(description='Department view type.')
    background_color = graphene.String(description='Background Color.')
    status = graphene.String(description="Department set active/disabled/delete")

class DepartmentMixin:
    @classmethod
    def clean_code(cls, instance, cleaned_input, errors):
        if instance.pk:
            code = instance.code
        else:
            department = models.Department.all_objects.last()
            if department:
                code = department.code
                if 'D' in code:
                    code = int(code.split('D')[1])
                    code = code + 1
                    code = 'D' + str(code)
                else:
                    cls.add_error(
                        errors, 'code',
                        'Department code could not generate.')
            else:
                code = 'D100000001'
        cleaned_input['code'] = code
        return cleaned_input


    @classmethod
    def clean_input(cls, info, instance, input, errors):
        cleaned_input = super().clean_input(info, instance, input, errors)

        if 'name' in cleaned_input:
            slug = slugify(cleaned_input['name'])
        elif instance.pk:
            slug = instance.slug
        else:
            cls.add_error(errors, 'name', 'This field cannot be blank.')
            return cleaned_input
        cleaned_input['slug'] = slug

        query = models.Department.all_objects.filter(slug=slug)
        query = query.exclude(pk=getattr(instance, 'pk', None))
        if query.exists():
            cls.add_error(
                errors, 'name',
                'Department already exists with this name.')

        if input.get('icon'):
            image_data = info.context.FILES.get(input['icon'])
            validate_image_file(cls, image_data, 'icon', errors)

        if input.get('large_icon'):
            image_data = info.context.FILES.get(input['large_icon'])
            validate_image_file(cls, image_data, 'large_icon', errors)

        if input.get('default_image'):
            image_data = info.context.FILES.get(input['default_image'])
            validate_image_file(cls, image_data, 'default_image', errors)
        clean_seo_fields(cleaned_input)
        cls.clean_code(instance, cleaned_input, errors)
        return cleaned_input

    @classmethod
    def save(cls, info, instance, cleaned_input):
        instance.save()
        if cleaned_input.get('icon'):
            create_department_default_thumbnails.delay(instance.pk, 'icon')

        if cleaned_input.get('large_icon'):
            create_department_default_thumbnails.delay(instance.pk, 'large_icon')

        if cleaned_input.get('default_image'):
            create_department_default_thumbnails.delay(instance.pk, 'default_image')


class DepartmentCreate(DepartmentMixin, ModelMutation):
    class Arguments:
        input = DepartmentInput(
            required=True, description='Fields required to create a department.')

    class Meta:
        description = 'Creates a new department.'
        model = models.Department

    @classmethod
    @permission_required('products')
    @role_required([UserType.ADMIN.value])
    def mutate(cls, root, info, **data):
        return super().mutate(root, info, **data)

class DepartmentUpdate(DepartmentMixin ,ModelMutation):

    class Arguments:
        input = DepartmentInput(
            required=True, description='Fields required to update a department.')
        id = graphene.ID(description='ID of the department')

    class Meta:
        description = 'Update a department.'
        model = models.Department

    @classmethod
    @permission_required('products')
    @role_required([UserType.ADMIN.value])
    def mutate(cls, root, info, **data):
        return super().mutate(root, info, **data)

class DepartmentChangeStatus(ModelStatusChangeMutation):

    class Arguments:
        status = graphene.String(description='New status of department')
        id = graphene.ID(description='ID of the department')
        cascade = graphene.Boolean(description='Option to delete via non-cascade')

    class Meta:
        description = 'Update a department status.'
        model = models.Department

    @classmethod
    @permission_required('products')
    @role_required([UserType.ADMIN.value])
    def mutate(cls, root, info, **data):
        return super().mutate(root, info, **data)



